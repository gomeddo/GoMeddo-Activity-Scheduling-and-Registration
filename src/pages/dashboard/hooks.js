...
export function useAgendaItems(reservations) {
  ...
  const sections = reservations.reduce((acc, reservation) => {
    ...
    const getCustomProperty = (property) => {
      const value = reservation.customProperties.get(property);
      return value !== undefined ? value : t(resources.label_unknown);
    };

    acc[timeOfDay].items.push({
      id: getCustomProperty("Name"),
      name: getCustomProperty("B25__Title__c"),
      room: getCustomProperty("Room_Name__c"),
      imageUrl: getCustomProperty("reservation_type_image__c"),
      instructor: formatName(getCustomProperty("Staff_Name__c")),
      time: `${new Date(
        getCustomProperty("B25__Start_Local_DateTime__c")
      ).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      })} - ${new Date(
        getCustomProperty("B25__End_Local_DateTime__c")
      ).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`,
      attendees: getCustomProperty("Room_Capacity__c"),
      location: getCustomProperty("City_Location__c"),
      center: getCustomProperty("Center_Name__c"),
      spaces: getCustomProperty("B25LP__Capacity__c"),
      start: new Date(getCustomProperty("B25__Start_Local_DateTime__c")),
      reservation,
    });
